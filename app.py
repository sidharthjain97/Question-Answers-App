...
def index():
    ...
    question_cur = db.execute("select a.id, a.question_text, a.asked_by_id, a.expert_id, b.name as asker_name, c.name as expert_name from questions a join users b on b.id = a.asked_by_id join users c on c.id = a.expert_id where a.answer_text is not null")
...
def question(question_id):
    ...
    db = get_db()
    question_cur = db.execute("select a.question_text, a.answer_text, a.asked_by_id, a.expert_id, b.name as asker_name, c.name as expert_name from questions a join users b on b.id=a.asked_by_id join users c on c.id=a.expert_id where a.id=?", [question_id])
...
def unanswered():
    ...
    question_cur = db.execute("select a.id, a.question_text, a.asked_by_id, b.name from questions a left join users b where a.asked_by_id = b.id and answer_text is NULL and expert_id= ? ", [user['id']])
    question_results = question_cur.fetchall()
...
def users():
    ...
    if user['admin'] == 0:
        return redirect(url_for('index'))    
    db = get_db()
    users_curr = db.execute('select id, name, expert, admin from users')
    users_result = users_curr.fetchall()
